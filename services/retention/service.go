...
type Service struct {
	MetaClient interface {
		...
		ShardIDsByNode(nodeId uint64) []uint64
	}
	TSDBStore interface {
		ShardIDs() []uint64
...
func (s *Service) run() {
	...
	for {
		...
		case <-ticker.C:
			...
			// Remove not exist in meta shards
			expectedShardIds := s.MetaClient.ShardIDsByNode(s.Node.ID)
			for _, id := range s.TSDBStore.ShardIDs() {
				exist := false
				for _, eid := range expectedShardIds {
					if eid == id {
						exist = true
					}
				}
				if !exist {
					...
					s.logger.Info(fmt.Sprintf("Shard ID %d , deleted.", id))
				}
			}
