...
import (
	...
	"github.com/colinsage/myts/stress/stress_client"
)
...
func (i *InsertStatement) SetVars(s *stressClient.StressTest) chan<- string {
	...
	i.next = make([]bool, 0)
	for n := len(i.stringers); n>0; n-- {
		i.next = append(i.next,true)
	}
...
func (i *InsertStatement) Run(s *stressClient.StressTest) {
	...
	for k := 0; k < i.Timestamp.Count; k++ {
		...
		change := false
		for t := range i.stringers {
			if t < i.TagCount  {
				if !change {
					serieId := k % i.series
					lastSerieId := serieId - 1
					if serieId == 0 {
						lastSerieId = i.series -1
					}
					divisor := 1
					for m:=i.TagCount-1; m>t; m-- {
						divisor *= i.Templates[m].numSeries()
					}
					last := lastSerieId/divisor
					curr := serieId /divisor
		...
		cur := i.stringers.Eval(i.time, i.last, i.next)
		point := fmt.Sprintf(i.TemplateString, cur...)
