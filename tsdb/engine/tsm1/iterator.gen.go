...
type booleanDescendingCacheCursor struct {
	cache struct {
		values Values
		pos    int
	}
	...
	tsc struct {
		index     *TimeSerieBlockIndexList
		values    []BooleanValue
		pos       int
	}
}

func newBooleanDescendingCacheCursor(seek int64, cacheValues Values, indexList *TimeSerieBlockIndexList) *booleanDescendingCacheCursor {
	c := &booleanDescendingCacheCursor{}
...
// peekCache returns the current time/value from the cache.
func (c *booleanDescendingCacheCursor) peekCache() (t int64, v bool) {
	if c.cache.pos < 0 || c.cache.pos >= len(c.cache.values) {
		return tsdb.EOF, false
	}
...
// close closes the cursor and any dependent cursors.
func (c *booleanDescendingCacheCursor) close() error {
	c.cache.values = nil
	return nil
}

// next returns the next key/value for the cursor.
func (c *booleanDescendingCacheCursor) next() (int64, interface{}) { return c.nextBoolean() }

// nextBoolean returns the next key/value for the cursor.
func (c *booleanDescendingCacheCursor) nextBoolean() (int64, bool) {
	ckey, cvalue := c.peekCache()
...
// nextCache returns the next value from the cache.
func (c *booleanDescendingCacheCursor) nextCache() {
	if c.cache.pos < 0 {
		return
	}
	c.cache.pos--
